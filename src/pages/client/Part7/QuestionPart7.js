...
const QuestionPart7 = () => {
  ...
  const fetchApi = async () => {
    try {
      const response = await axios.get(Summary.getQuestionPart7.url + params.id, {
        headers: {
  ...
  const handleOk = () => {
    ...
    setDeadline(Date.now() + 50 * 60 * 1000);
  };
  const handleCancel = () => {
    setIsModalOpen(false);
    if (audioRef.current) {
  ...
  return (
    <Card style={{ minHeight: '100vh', marginTop: '10vh', padding: '2rem', boxShadow: '0px 4px 12px rgba(0, 0, 0, 0.1)', position: 'relative' }}>
      ...
      <Row style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        {data.length > 0 && (
          <Col xxl={24} xl={24} xs={24} md={18} lg={12} style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', minWidth: '100%', justifyContent: 'center', alignItems: 'center' }}>
            ...
            {/* Nhóm các lựa chọn */}
            <Radio.Group
              name={`${data[currentQuestionIndex].questionNumber - 146}`}
              value={answers[data[currentQuestionIndex].questionNumber - 146]}
      ...
      <Notify time={50} question={data.length} showModal={showModal} isModalOpen={isModalOpen} handleOk={handleOk} handleCancel={handleCancel} />
    </Card>
  )
}
