...
const QuestionPart4 = () => {
  ...
  const fetchApi = async () => {
    try {
      const response = await axios.get(Summary.getQuestionPart4.url + params.id, {
        headers: {
  ...
  return (
    <Card style={{ minHeight: '100vh', marginTop: '10vh', padding: '2rem', boxShadow: '0px 4px 12px rgba(0, 0, 0, 0.1)', position: 'relative' }}>
      {/* Phần audio */}
      <Row justify="center" style={{ marginBottom: '2rem', display: 'none' }}>
      ...
      <Notify time={6} question={data.length} showModal={showModal} isModalOpen={isModalOpen} handleOk={handleOk} handleCancel={handleCancel} />
    </Card>
  )
}

export default QuestionPart4
