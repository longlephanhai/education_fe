...
const QuestionPart5 = () => {
  ...
  const fetchApi = async () => {
    try {
      const response = await axios.get(Summary.getQuestionPart5.url + params.id, {
        headers: {
      ...
      <Row style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        {data.length > 0 && (
          <Col xxl={24} xl={24} xs={24} md={18} lg={12} style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', minWidth: '100%', justifyContent: 'center', alignItems: 'center' }}>
            ...
            {/* Nhóm các lựa chọn */}
            <Radio.Group
              name={`${data[currentQuestionIndex].questionNumber - 100}`}
              value={answers[data[currentQuestionIndex].questionNumber - 100]}
